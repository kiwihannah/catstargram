...
function validate_pw() {
  ...
  if (!regExp.test($('#reg-user-pw').val())) {
    $('#pw-alert-1').empty();
    $('#pw-alert-1').append(
      "<p style='color:red'>최소 3자 이상, 알파벳 대소문자(a~z, A~Z), 숫자(0~9)</p>"
    );
    ...
    $('#pw-alert-1').empty();
    $('#pw-alert-1').append(
      "<p style='color:red'>비밀번호는 아이디와 동일하게 입력할 수 없다.</p>"
    );
    $('#reg-user-pw').val('').focus();
    $('#btn-login').classList.add('disabled');
  } else {
...
function dup_check() {
  ...
  if (validate_id(user_id)) {
    $.ajax({
      ...
      success: function (response) {
        if (response['msg'] === -1) {
          ...
          $('#id-alert').empty();
          $('#id-alert').append(
            "<p style='color:green'>가입 가능한 아이디 입니다.</p>"
          );
        }
      },
    });
    ...
    $('#id-alert').empty();
    $('#id-alert').append(
      "<p style='color:red'>3자 이상, 알파벳 대소문자(a~z, A~Z), 숫자(0~9)</p>"
    );
